...
const TopArea = ({ setUser }: TopAreaProps): JSX.Element => {
	const { changeTheme, isLightMode } = useContext(ThemeContext);
	const [isEmpty, setIsEmpty] = useState<boolean>(false);
	const [notFound, setNotFound] = useState<boolean>(false);
	const usernameRef = useRef<HTMLInputElement>(null);
	const [inputUser] = useState("bryanAguilar001");
	...
	const handleSubmit = (): void => {
		...
		setIsEmpty(false);
		getUserFromGitHub(usernameRef.current.value);
	};

	const getUserFromGitHub = async (username: string): Promise<void> => {
		...
		const user: UserProps = {
			pfp: data.avatar_url,
			name: data.name,
			joinedAt: formatDate(new Date(data.created_at)),
			username: data.login,
			bio: data.bio,
	...
	useEffect(() : void => {
		getUserFromGitHub(inputUser);
	}, [inputUser]);

	return (
		<Container>
			<ThemeArea>
				<svg width="124" height="21" viewBox="0 0 126 21" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path
			...
			>
				...
				<Input ref={usernameRef} name="username" id="username" type="text" placeholder="Search github's username" />
        {isEmpty && <Warn>Enter User</Warn>}
